...
export function StoreMenuLocaleBar() {
  ...
  const { switchLocale, loading, locale } = useSwitchLocale();
  ...
  return (
    <div
      className="pointer-events-auto fixed inset-x-0 top-0 z-[100] flex justify-center px-4 pt-[max(0.75rem,env(safe-area-inset-top))]"
      role="group"
        ...
        {LOCALE_ORDER.map((code) => {
          ...
          return (
            <button
              ...
              className={`cursor-pointer rounded-full px-3 py-1.5 text-[11px] font-medium transition disabled:opacity-50 sm:px-3.5 sm:text-xs ${
                isActive
